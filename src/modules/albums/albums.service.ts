...
export class AlbumsService {
  ...
  async getTracks(ids: string[]): Promise<Track[]> {
    return await Promise.all(ids.map((id) => this.tracksService.findOne(id)));
  }

  async create(body: CreateAlbumInput, token: string): Promise<Album> {
    const album = this.httpService
      .post(process.env.ALBUMS_API, body, createAxiosConfigWithToken(token))
      .pipe(map(({ data }) => data));
